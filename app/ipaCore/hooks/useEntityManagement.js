...
import {
  getClickEvent,
  getMapTypes,
  getSelectedGraphicReference,
  getSelectedStructure,
} from '../redux/pageComponentState';
import {
...
export const useNewEntityManagement = ({ portContext, mapInstance }) => {
  ...
  const [currentElementType, namedPathDict, namedPath] = useMemo(() => {
    ...
    const namedPathDict = Object.assign(
      {},
      ...namedPaths.map((p) => ({ [p.state]: p })),
    );
  ...
  const handleCancelElement = (elementToRemove, draftingType, namedPath) => {
    ...
    if (elementToRemove.isDraft) {
      const filteredSites = currentEntities.filter(
        (s) => s[idKey] !== elementToRemove[idKey],
      );
  ...
  useEffect(() => {
    const [draftingType, draftElement] =
      Object.entries(currentState?.context?.data || {})
        .map(([k, v]) => {
  ...
  useEffect(() => {
    if (
      previousIsSelectingPosition &&
      isSelectingPosition &&
      clickEvent?.ground &&
      !_.isEqual(previousClick, clickEvent)
    ) {
      dispatch(
        setSelectedCoordinate([
          clickEvent.ground.longitude,
          clickEvent.ground.latitude,
        ]),
      );
    ...
    if (
      selectedCoordinate?.length &&
      !isSelectingPosition &&
      draftNamedPath?.feature === 'mesh'
    ) {
      const [centerLng, centerLat] = selectedCoordinate;

      const parentNamedPath = currentState.context.namedPaths[0].find(
        (p) => p.state === draftNamedPath.parentState,
      );
      const currentParentId = currentState.context[parentNamedPath.idKey];

      const positionMatchesParent = clickEvent.elements.some(
        (el) =>
          el?.extra?.attributeName === parentNamedPath.idKey &&
          el?.extra?.attributeValue === currentParentId,
      );
      ...
      if (selectedGraphicReference && selectedGraphicReference._id) {
        ...
        console.log(
          'Retrieved geometry info for graphic:',
          graphicId,
          geometryInfo,
        );
      }
    ...
    if (
      selectedCoordinate?.length &&
      !isSelectingPosition &&
      draftNamedPath?.feature === 'polygon'
    ) {
      const [centerLng, centerLat] = selectedCoordinate;
