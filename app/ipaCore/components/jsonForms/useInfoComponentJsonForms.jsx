...
const buildUiFromSchema = (schema) => {
  ...
  const entries = Object.entries(schema.properties).sort(
    ([ak, av], [bk, bv]) => {
      const ao = av.propertyOrder ?? 999,
        bo = bv.propertyOrder ?? 999;
...
export function useInfoComponentJsonForms({
  schema,
  layouts,
  ...
  const optionsResolver = useCallback(async ({ path /*, data*/ }) => {
    if (path.endsWith('/siteType')) {
      ...
      return [
        { const: '900MW', title: '900 MW' },
        { const: '1300MW', title: '1300 MW' },
        { const: '1450MW', title: '1450 MW' },
      ];
    }
