	...
	async initialize() {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let modelInfo = manifest.files.find(f => !f.hasOwnProperty('references'))
				await IafScriptEngine.setVar("model_manifest", modelInfo)
	...
	async #createBIMCollectionVersion() {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const [bimModel, bimpkFileId, bimpkFileVersionId, packagename, packagenameShort, modelManifest] = await Promise.all([
					IafScriptEngine.getVar("bim_model"),
					...
					IafScriptEngine.getVar('model_manifest')
				])
				const newModelVer = {
					...
					_userAttributes: {
						...
						model: {
							source: modelManifest.source ? modelManifest.source : 'Unknown',
							originalSource: modelManifest.originalSource ? modelManifest.originalSource : 'None'
						}
					}
				}
	...
	async #createBIMCollections() {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const [packagename, packagenameShort, bimpkFileId, bimpkFileVersionId, modelManifest] = await Promise.all([
					IafScriptEngine.getVar("package_name"),
					IafScriptEngine.getVar("package_name_short"),
					IafScriptEngine.getVar("bimpk_fileid"),
					IafScriptEngine.getVar("bimpk_fileVersionId"),
