# Portfolio Developer Update Guide

The Portfolio Developer Update Guide will want you through updating a 2.0.0 version of the template to 2.1.0.
...
> **BEFORE UPDATING Portfolio**: You must first make sure your Twinit VS Code extension is version 6.2.2 or newer. If you do not then the update process will not correctly deploy the template scripts to your Quick View Manager project.

Updating to 2.1.0 is seamless to users of Portfolio. User will not be impacted during the update. The previous version of the web client will continue to function during the update. Once the new web client is deployed they will see the updated user interface.
...
> **IMPORTANT**: These steps will update the Portfolio template and web client. To take advantage of all new updates, additional steps are required by Admin users in each Portfolio project. Be sure to consult the [User Guide](../user%20guide/README.md) for more information.
